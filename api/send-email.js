  ...
  if (req.method === 'POST') {
    try {
      ...
      if (templateData && templateData.puntos_por_vencer && templateData.puntos_por_vencer > 0 && templateData.fecha_vencimiento) {
          htmlBloqueVencimiento = `
              <div style="border: 1px solid #ffc107; padding: 10px; margin-top: 15px; background-color: #fff3cd; border-radius: 5px;">
                <p style="margin: 0;"><b>¡Atención!</b> Tienes <b>${templateData.puntos_por_vencer} puntos</b> que están próximos a vencer el día <b>${templateData.fecha_vencimiento}</b>. ¡No dejes que se pierdan!</p>
              </div>
          `;
      }
